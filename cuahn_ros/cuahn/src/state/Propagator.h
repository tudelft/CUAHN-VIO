...
#ifndef CUAHN_STATE_PROPAGATOR_H
#define CUAHN_STATE_PROPAGATOR_H
...
public:
  ...
  Propagator(NoiseManager noises, double gravity_mag) : _noises(noises) {
    ...
    Q.block(0, 0, 3, 3) = _noises.sigma_w_2 * Eigen::Matrix<double, 3, 3>::Identity();
    Q.block(3, 3, 3, 3) = _noises.sigma_a_2 * Eigen::Matrix<double, 3, 3>::Identity();
    Q.block(6, 6, 3, 3) = _noises.sigma_ab_2 * Eigen::Matrix<double, 3, 3>::Identity();
    Q.block(9, 9, 3, 3) = _noises.sigma_wb_2 * Eigen::Matrix<double, 3, 3>::Identity();
    Q.block(12, 12, 3, 3) = 1.0e-04 * Eigen::Matrix<double, 3, 3>::Identity();

    last_prop_time_offset = 0.0;
    _gravity << 0.0, 0.0, -gravity_mag; // in forward-left-up frame, should be -9.8
    _muw_vec << 0.0, 0.0, -1.0; // in forward-left-up frame, should be -1.0 // the normal vector of the ground plane, pointing down to the earth
    _ezT << 0.0, 0.0, 1.0;
    _I33 = Eigen::Matrix<double, 3, 3>::Identity();
  }
   ...
   */
  void propagate_with_imu(std::shared_ptr<State> state, double timestamp);
  ...
  void set_const_Jacobian(std::shared_ptr<State> state) {
    _J_vc_v  = state->c_RotMtrx_i;
    _J_vc_bw = state->c_RotMtrx_i * skew_x(state->i_tVec_i2c);
    _J_wc_bw = - state->c_RotMtrx_i;
    printf(GREEN "Set constant Jacobian matrices.\n" RESET);
  }
...
protected:
  ...
  /// fixed propagation noise matrix
  Eigen::Matrix<double, 15, 15> Q = Eigen::Matrix<double, 15, 15>::Zero();

  Eigen::Matrix<double, 27, 27> F = Eigen::Matrix<double, 27, 27>::Zero();
  Eigen::Matrix<double, 27, 15> Fw = Eigen::Matrix<double, 27, 15>::Zero();

  // constant valuables (start with _)
  Eigen::Matrix<double, 3, 1> _muw_vec; // normal vector of the ground expressed in world frame
  Eigen::Matrix<double, 1, 3> _ezT;
  Eigen::Matrix<double, 3, 3> _I33;

  Eigen::Matrix<double, 3, 3> _J_vc_v;
  Eigen::Matrix<double, 3, 3> _J_vc_bw;
  Eigen::Matrix<double, 3, 3> _J_wc_bw;

  // the following valuables are needed for both state propagation and calculating F and Fw matrices.
  // They are calculated only in predict_and_compute() function and used again for state propagation and calculating F and Fw matrices.
  Eigen::Matrix<double, 3, 1> wc_vec;
  Eigen::Matrix<double, 3, 1> vc_vec;
  Eigen::Matrix<double, 3, 1> muc_vec;
  double dc;
  Eigen::Matrix<double, 3, 1> pt_upperLeft;
  Eigen::Matrix<double, 3, 1> pt_bottomLeft; 
  Eigen::Matrix<double, 3, 1> pt_bottomRight;
  Eigen::Matrix<double, 3, 1> pt_upperRight;

};

} // namespace cuahn

#endif // CUAHN_STATE_PROPAGATOR_H
