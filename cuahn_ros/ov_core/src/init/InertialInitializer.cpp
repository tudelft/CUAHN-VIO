...
bool InertialInitializer::initialize_with_imu_CUAHN(double &time0, Eigen::Matrix<double, 4, 1> &q_I0toW, Eigen::Matrix<double, 3, 1> &b_g0,
                                                    Eigen::Matrix<double, 3, 1> &I_v_I0, Eigen::Matrix<double, 3, 1> &b_a0,
                                                    Eigen::Matrix<double, 3, 1> &I_p_I0, double init_height, bool wait_for_jerk) {
  ...
  // This is the case when we have started up moving, and thus we need to wait for a period of stationary motion
  if (a_var_2to1 > _imu_excite_threshold && wait_for_jerk) {
    printf(YELLOW "[INIT-IMU]: to much IMU excitation, above threshold %.4f > %.4f Wait to initialize when stationary...\n" RESET, a_var_2to1, _imu_excite_threshold);
    return false;
  }
  ...
  // Create our state variables
  Eigen::Matrix<double, 4, 1> q_ItoW = rot_2_Ham_quat(Ro.transpose());

  // Set our biases equal to our noise (subtract our gravity from accelerometer bias)
  Eigen::Matrix<double, 3, 1> ba = a_avg_2to1 - Ro * _gravity;
  Eigen::Matrix<double, 3, 1> bg = w_avg_2to1;

  Eigen::Matrix<double, 3, 1> W_p_I0(0.0, 0.0, init_height);

  // Set our state variables
  time0 = window_2to1.at(window_2to1.size() - 1).timestamp;
  I_p_I0 = Ro * W_p_I0; // position vector: point from the origin of World frame to IMU, expressed in IMU frame
  q_I0toW = q_ItoW;
  I_v_I0 = Eigen::Matrix<double, 3, 1>::Zero();
  b_a0 = ba;
  b_g0 = bg;

  printf(YELLOW "Initialize EKF state with stationary IMU measurements!\n", RESET);
  printf(YELLOW "Initial distance to ground: %.3f\n", RESET, init_height);
