...
//***************functions for Hamilton Quaternion of CUAHN ****************
/**
 * @brief Integrated quaternion from a rotation vector
 */
inline Eigen::Matrix<double, 4, 4> Ham_quat_update(Eigen::Matrix<double, 3, 1> rot_vec) {
  Eigen::Matrix<double, 4, 4> qR_Mtrx;

  double rot_angle = rot_vec.norm();
  Eigen::Matrix<double, 3, 1> dqv = sin(rot_angle*0.5) * rot_vec / rot_angle;

  qR_Mtrx = Eigen::MatrixXd::Identity(4, 4) * cos(rot_angle*0.5);
  qR_Mtrx.block(1, 1, 3, 3) = qR_Mtrx.block(1, 1, 3, 3) + skew_x(-dqv);
  qR_Mtrx.block(0, 1, 1, 3) = -dqv.transpose();
  qR_Mtrx.block(1, 0, 3, 1) = dqv;
  
  return qR_Mtrx;
}

/**
 * @brief Converts Hamilton quaterion to SO(3) rotation matrix (rotate a vec from local frame to global frame)
 *
 * @param[in] q Hamilton quaternion
 * @return 3x3 SO(3) rotation matrix
 */
inline Eigen::Matrix<double, 3, 3> Ham_quat_2_Rot(const Eigen::Matrix<double, 4, 1> &q) {
  Eigen::Matrix<double, 3, 3> Rot = Eigen::MatrixXd::Identity(3, 3) * (q(0, 0)*q(0, 0) - (q(1,0)*q(1,0)+q(2,0)*q(2,0)+q(3,0)*q(3,0)))
                                    + 2 * q.block(1, 0, 3, 1) * q.block(1, 0, 3, 1).transpose() + 2 * q(0, 0) * skew_x(q.block(1, 0, 3, 1));
  return Rot;
}

/**
 * @brief Returns a Hamilton quaternion from a rotation matrix
 *
 * @param[in] rot 3x3 rotation matrix
 * @return 4x1 quaternion
 */
inline Eigen::Matrix<double, 4, 1> rot_2_Ham_quat(const Eigen::Matrix<double, 3, 3> &rot) {
  Eigen::Matrix<double, 4, 1> q;
  double T = rot.trace();
  assert(T > -1.0); // always for SO(3)?
...
inline Eigen::Matrix<double, 3, 3> Jr_theta(Eigen::Matrix<double, 3, 1> theta) {

  double norm = theta.norm();
  Eigen::Matrix<double, 3, 3> Jr_theta_Mtrx;
  Jr_theta_Mtrx = Eigen::MatrixXd::Identity(3, 3) - (1 - cos(norm)) / (norm*norm) * skew_x(theta) +
                  (norm - sin(norm)) / (norm*norm*norm) * skew_x(theta) * skew_x(theta);
  return Jr_theta_Mtrx;
}

inline Eigen::Matrix<double, 4, 1> rotVec_2_Ham_quat(Eigen::Matrix<double, 3, 1> rot_vec) {
  Eigen::Matrix<double, 4, 1> q;
  double norm = rot_vec.norm();
  q(0, 0) = cos(norm*0.5);
  q.block(1, 0, 3, 1) = sin(norm*0.5) * rot_vec / norm;
  return q;
}
