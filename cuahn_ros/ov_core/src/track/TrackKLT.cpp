...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  database->num_updated_feats = (int)good_left.size();
  ...
  // // Timing information
  // printf("[TIME-KLT]: %.4f milliseconds for pyramid\n",(rT2-rT1).total_microseconds() * 1e-3);
  // printf("[TIME-KLT]: %.4f milliseconds for detection\n",(rT3-rT2).total_microseconds() * 1e-3);
  // printf("[TIME-KLT]: %.4f milliseconds for temporal klt\n",(rT4-rT3).total_microseconds() * 1e-3);
  printf("[TIME-KLT]: %.4f milliseconds for feature DB update (%d features)\n",(rT5-rT4).total_microseconds() * 1e-3, (int)good_left.size());
  // printf("[TIME-KLT]: %.4f milliseconds for total\n",(rT5-rT1).total_microseconds() * 1e-3);
}
