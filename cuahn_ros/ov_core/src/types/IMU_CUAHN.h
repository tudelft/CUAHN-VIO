...
#ifndef OV_TYPE_TYPE_IMU_CUAHN_H
#define OV_TYPE_TYPE_IMU_CUAHN_H
 ...
 *
 * Contains a PoseCUAHN, Vec velocity, Vec gyro bias, and Vec accel bias.
...
public:
  IMU() : Type(15) {
    ...
    _pose = std::shared_ptr<PoseCUAHN>(new PoseCUAHN());// p, q
  ...
  void set_local_id(int new_id) override {
    ...
    _ba->set_local_id(_v->id() + ((new_id != -1) ? _v->size() : 0));
    _bg->set_local_id(_ba->id() + ((new_id != -1) ? _ba->size() : 0));
  }

  /**
   * @brief Performs update operation using HamQuat update for orientation, then vector updates for
  ...
  /// Pose type access
  std::shared_ptr<PoseCUAHN> pose() { return _pose; }

  /// Quaternion type access
  std::shared_ptr<HamQuat> q() { return _pose->q(); }
...
protected:
  /// Pose subvariable
  std::shared_ptr<PoseCUAHN> _pose;
  ...
  void set_value_internal(const Eigen::MatrixXd &new_value) {
    ...
    _ba->set_value(new_value.block(10, 0, 3, 1));
    _bg->set_value(new_value.block(13, 0, 3, 1));
  ...
  void set_fej_internal(const Eigen::MatrixXd &new_value) {
    ...
    _ba->set_fej(new_value.block(10, 0, 3, 1));
    _bg->set_fej(new_value.block(13, 0, 3, 1));
...
#endif // OV_TYPE_TYPE_IMU_CUAHN_H
