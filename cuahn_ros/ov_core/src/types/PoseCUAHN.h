...
#ifndef OV_TYPE_TYPE_POSECUAHN_H
#define OV_TYPE_TYPE_POSECUAHN_H
 ...
 *
 * Internally we use a HamQuat quaternion representation for the orientation and 3D Vec position.
 * Please see HamQuat for details on its update procedure and its left multiplicative error.
 */
class PoseCUAHN : public Type {
...
public:
  PoseCUAHN() : Type(6) {
    ...
    _q = std::shared_ptr<HamQuat>(new HamQuat());
  ...
  void set_local_id(int new_id) override {
    _id = new_id;
    _p->set_local_id(new_id);
    _q->set_local_id(new_id + ((new_id != -1) ? _p->size() : 0));
  }

  /**
   * @brief Update q and p using a the HamQuat update for orientation and vector update for position
   *
  ...
  void update(const Eigen::VectorXd &dx) override {
    ...
    // Update orientation
    newX.block(3, 0, 4, 1) = ov_core::quat_multiply(dq, quat());
    // Update position
    newX.block(0, 0, 3, 1) += dx.block(3, 0, 3, 1);
  ...
  std::shared_ptr<Type> clone() override {
    auto Clone = std::shared_ptr<PoseCUAHN>(new PoseCUAHN());
  ...
  std::shared_ptr<HamQuat> q() { return _q; }
...
protected:
  ...
  void set_value_internal(const Eigen::MatrixXd &new_value) {
    ...
    _p->set_value(new_value.block(0, 0, 3, 1));

    // Set position value
    _q->set_value(new_value.block(3, 0, 4, 1));
  ...
  void set_fej_internal(const Eigen::MatrixXd &new_value) {
    ...
    _p->set_fej(new_value.block(0, 0, 3, 1));

    // Set position fej value
    _q->set_fej(new_value.block(3, 0, 4, 1));
...
#endif // OV_TYPE_TYPE_POSECUAHN_H