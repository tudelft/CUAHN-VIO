...
public:
  ...
  void callback_odometry(const nav_msgs::OdometryPtr &msg) {
    ...
    p_BinW << msg->pose.pose.position.x, msg->pose.pose.position.y, msg->pose.pose.position.z;
    Euler_WtoB << msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z;
    W_v_B << msg->twist.twist.linear.x, msg->twist.twist.linear.y, msg->twist.twist.linear.z;
    write_onboard();
  }
...
protected:
  ...
  void write_onboard() {
    ...
    // pose
    outfile.precision(6);
    outfile << p_BinW.x() << " " << p_BinW.y() << " " << p_BinW.z() << " " 
            << Euler_WtoB.x() << " " << Euler_WtoB.y() << " " << Euler_WtoB.z() << " " 
            << W_v_B.x() << " " << W_v_B.y() << " " << W_v_B.z() << std::endl;
  ...
  Eigen::Vector3d p_BinW;
  Eigen::Vector3d Euler_WtoB;
  Eigen::Vector3d W_v_B; // velocity expressed in world frame
